...
using eAppointmentServer.Application.Features.Doctors.GetAllDoctorsByDepartmentId;
...
public sealed class DoctorController(IMediator mediator) : ApiController(mediator)
{
  ...
  [HttpPost]
  public async Task<IActionResult> GetAllDoctorsByDepartmentId(GetAllDoctorsByDepartmentIdQuery request, CancellationToken cancellationToken)
  {
    var response = await _mediator.Send(request, cancellationToken);
    return StatusCode(response.StatusCode, response);
  }
