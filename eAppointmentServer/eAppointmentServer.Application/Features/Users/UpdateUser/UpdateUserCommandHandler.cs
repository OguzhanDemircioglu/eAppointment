  ...
  IMapper mapper) : IRequestHandler<UpdateUserCommand, Result<string>>
{
  public async Task<Result<string>> Handle(UpdateUserCommand request, CancellationToken cancellationToken)
  {
    ...
    if (user is null)
    {
      return Result<string>.Failure(ResultMessages.NO_USER);
    }
    ...
    if(user.Email != request.Email)
    {
      ...
      {
        return Result<string>.Failure(ResultMessages.SAME_EMAIL);
      }
    }
    ...
    if(user.UserName != request.UserName)
    {
      ...
      {
        return Result<string>.Failure(ResultMessages.SAME_USER);
      }
    }       
